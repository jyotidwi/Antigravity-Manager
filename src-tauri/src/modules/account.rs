...
mod tests {
    ...
    #[test]
    fn test_is_enterprise_client_matching() {
        assert!(is_enterprise_client(Some("antigravity_enterprise")));
        assert!(is_enterprise_client(Some("  ANTIGRAVITY_ENTERPRISE  ")));
        assert!(!is_enterprise_client(Some("other_client")));
        assert!(!is_enterprise_client(Some("   ")));
        assert!(!is_enterprise_client(None));
    }

    #[test]
    fn test_normalize_project_id_filters_empty() {
        assert_eq!(
            normalize_project_id(Some(" graphic-autumn-9sk6k ")),
            Some("graphic-autumn-9sk6k".to_string())
        );
    ...
    #[test]
    fn test_fallback_cached_quota_on_429_unknown_error() {
        let mut account = Account::new(
            "acc-1".to_string(),
            "rate-limited@example.com".to_string(),
            TokenData::new(
                "access".to_string(),
                "refresh".to_string(),
                3600,
                Some("rate-limited@example.com".to_string()),
                None,
                None,
                true,
            ),
        );
        let mut cached = QuotaData::new();
        cached.add_model(crate::models::quota::ModelQuota {
            name: "gemini-2.5-pro".to_string(),
            percentage: 88,
            reset_time: "2026-03-16T00:00:00Z".to_string(),
        ...
        account.quota = Some(cached.clone());

        let err = crate::error::AppError::Unknown(
            "API Error: 429 Too Many Requests - RESOURCE_EXHAUSTED".to_string(),
        );

        let recovered = recover_cached_quota_on_rate_limit(&account, &err);
        assert!(recovered.is_some(), "429 should reuse cached quota");
        assert_eq!(
            recovered.expect("quota should exist").models.len(),
            cached.models.len()
        );
    }

    #[test]
    fn test_no_fallback_for_non_rate_limit_error() {
        let mut account = Account::new(
            "acc-2".to_string(),
            ...
            TokenData::new(
                ...
                "refresh".to_string(),
                3600,
                Some("normal@example.com".to_string()),
                None,
                None,
                true,
            ),
        ...
        let err = crate::error::AppError::OAuth("invalid_grant".to_string());
        assert!(
            recover_cached_quota_on_rate_limit(&account, &err).is_none(),
            "non-rate-limit errors should not use cached quota"
        );
    }

    #[test]
    fn test_detect_validation_required_error() {
        let err = crate::error::AppError::Unknown(
            "Further action is required to use Antigravity".to_string(),
        );
        assert!(is_validation_required_error(&err));

        let non_validation = crate::error::AppError::OAuth("invalid_grant".to_string());
        assert!(!is_validation_required_error(&non_validation));
    }

    #[test]
    fn test_detect_account_access_blocked_message() {
        assert!(is_account_access_blocked_message(
            "oauth2: \"unauthorized_client\" \"Unauthorized\""
        ));
        assert!(is_account_access_blocked_message(
            "Further action is required to use Antigravity"
        ));
        assert!(!is_account_access_blocked_message("connection reset by peer"));
    }

    #[test]
    fn test_format_switch_refresh_error_for_unauthorized_client() {
        let message = format_switch_refresh_error("oauth2: \"unauthorized_client\" \"Unauthorized\"");
        assert!(message.contains("OAuth client"));
        assert!(message.contains("sign in again"));
    }

    #[test]
    fn test_format_rate_limit_block_reason_contains_resource_exhausted() {
        let err = crate::error::AppError::Unknown(
            "API Error: 429 Too Many Requests - RESOURCE_EXHAUSTED".to_string(),
        );
        let reason = format_rate_limit_block_reason(&err);
        assert!(reason.to_lowercase().contains("resource_exhausted"));
    }
}
